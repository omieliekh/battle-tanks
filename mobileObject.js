								...
								if ('brick' === currCell[m].state) {
									...
									if (0 === this.dirn.x) {
										if (fldBricks[ cur.x+0.25 ] && fldBricks[ cur.x+0.25 ][ cur.y ]) {
											...
											/*if (-1 == this.dirn.y && fldBricks[ cur.x+0.25 ][ cur.y+0.25 ]) {//�����
												clList.push(fldBricks[ cur.x+0.25 ][ cur.y+0.25 ]);
											} else if (fldBricks[ cur.x+0.25 ][ cur.y-0.25 ]) { // ����
												clList.push(fldBricks[ cur.x+0.25 ][ cur.y-0.25 ]);
											}*/
										}
										if (fldBricks[ cur.x-0.25 ] && fldBricks[ cur.x-0.25 ][ cur.y ]) {
										...
										if (fldBricks[ cur.x ] && fldBricks[ cur.x ][ cur.y+0.25 ]) {
											...
											/*if (-1 == this.dirn.x && fldBricks[ cur.x+0.25 ] && fldBricks[ cur.x+0.25 ][ cur.y+0.25 ]) {//�����
												clList.push(fldBricks[ cur.x+0.25 ][ cur.y+0.25 ]);
											} else if (fldBricks[ cur.x-0.25 ] && fldBricks[ cur.x-0.25 ][ cur.y-0.25 ]) { // ������
												clList.push(fldBricks[ cur.x-0.25 ][ cur.y-0.25 ]);
											}*/
										}
